...
for i in range(len(words)/37):			#Loop amount of data point times
	...
	accx[i] = float(words[(i*37)+26])	#Add 20, 22, 24 for non calculated values
	accy[i] = float(words[(i*37)+28])	#Add 14, 16, 18 for the calculated values
	accz[i] = float(words[(i*37)+30])
